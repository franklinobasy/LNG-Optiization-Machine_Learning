...
class App(tk.Tk):
    ...
    def load_to_input(self):
        ...
        self.lng.set(values[1])
        self.mr_return_temp.set(values[2])
        self.IGV.set(values[3])
        self.HMR_flow.set(values[4])
        self.HR_MR_Ratio.set(values[5])
        self.pNitrogen.set(values[6])
        self.pMethane.set(values[7])
        self.pEthane.set(values[8])
        self.pPropane.set(values[9])
        self.pButane.set(values[10])
        self.ambient_cond.set(values[11])
        self.kettle_level.set(values[12])
        print(values)
        self.counter += 1
    ...
    def setUpOutput(self):
        ...
        self.col1 = tk.Frame(self.output_frame, width=600, height=300)   
        self.col2 = tk.Frame(self.output_frame, width=200, height=300, bg = 'grey')
        ...
        #
        # Column 1 - Text
        self.display = tk.Text(self.col1, height = 140, width=580)
        self.display.pack()
    ...
    def optimize(self):
        ...
        _input = [igv, hmr_flow, hr_mr_ratio, pN, pM, pE, pP, pB, ambient_cond, kettle_level]
        _output = np.array([[lng_flow, mr_return_temp]])

        p = Optimize(self.model.predict, _input, App.search_space, _output)
        text = "---------------------\noptimizing\n"
        ...
        increase = percentage_increase(approximate_output, _output)
        
        text += f"Aproximate LNG Flow: {approximate_output[0, 0]} ({increase[0, 0]:.4f})%\n"
        text += f"Aproximate LNG Rundown Temperature: {approximate_output[0, 1]} ({increase[0, 1]:.4f})%\n"
